import os
from voice_interaction.GPT_SoVITS import utils
...
def run(rank, n_gpus, hps):
    ...
    if rank == 0:
        ...
        logger.info(hps)
        # emotion.check_git_hash(hps.s2_ckpt_dir)
    ...
    # if rank == 0:
    #     eval_dataset = TextAudioSpeakerLoader(hps.test.validation_files, hps.test, val=True)
    ...
    try:  # 如果能加载自动resume
        ...
        # _, _, _, epoch_str = emotion.load_checkpoint(emotion.latest_checkpoint_path(hps.model_dir, "G_*.pth"), net_g, optim_g,load_opt=0)
        _, _, _, epoch_str = utils.load_checkpoint(
