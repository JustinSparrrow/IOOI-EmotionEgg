...
class DistributedBucketSampler(Sampler[T_co]):
    ...
    def __init__(
        ...
        # is no need to drop any test, since the dataset will be split equally.
        if (
            self.drop_last and len(self.dataset) % self.num_replicas != 0
        ):  # type: ignore[arg-type]
            # Split to nearest available length that is evenly divisible.
            # This is to ensure each rank receives the same amount of test when
            # using this Sampler.
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        if not self.drop_last:
            ...
            # remove tail of test to make it evenly divisible.
