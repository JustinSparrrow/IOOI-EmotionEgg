...
def buf_to_float(x, n_bytes=2, dtype=np.float32):
    ...
    This is primarily useful when loading integer-valued wav test
    into numpy arrays.

    Parameters
    ----------
    x : np.ndarray [dtype=int]
        The integer-valued test buffer
    ...
    x_float : np.ndarray [dtype=float]
        The input test buffer cast to floating point
    """

    # Invert the scale of the test
    scale = 1.0 / float(1 << ((8 * n_bytes) - 1))

    # Construct the format string
    fmt = "<i{:d}".format(n_bytes)

    # Rescale and format the test buffer
